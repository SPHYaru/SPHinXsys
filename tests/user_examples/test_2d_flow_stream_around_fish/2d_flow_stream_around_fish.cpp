...
int main(int ac, char *av[])
{
	...
	ContactRelation fish_observer_contact_relation_fish(fish_observer, { &fish_body});
	ContactRelation fish_observer_contact_relation_fluid(fish_observer, { &water_block });
	//----------------------------------------------------------------------
	...
	//SimpleDynamics<fluid_dynamics::InflowVelocityCondition<FreeStreamVelocity>, BodyAlignedBoxByCell> emitter_buffer_inflow_condition(emitter_buffer, fish_body);
	...
	/** We can output a method-specific particle data for debug */
	fish_body.addBodyStateForRecording<Real>("ActiveContractionStress");
	/** Compute the active contraction stress */
	SimpleDynamics<MyocardiumActivation> myocardium_activation(fish_body);
	/** Stress relaxation for the inserted body. */
	Dynamics1Level<solid_dynamics::Integration1stHalf> fish_body_stress_relaxation_first_half(fish_inner);
	...
	/** Pressure probe on gate. */
	ObservedQuantityRecording<Real> pressure_probe_fish("Pressure", io_environment, fish_observer_contact_relation_fluid);
	/** Interpolate the particle position in gate to move the observer accordingly. */
	InteractionDynamics<InterpolatingAQuantity<Vecd>>
		interpolation_observer_position(fish_observer_contact_relation_fish, "Position", "Position");
	//----------------------------------------------------------------------
	...
	int output_interval = screen_output_interval * 10;
	...
	while (GlobalStaticVariables::physical_time_ < End_Time)
	{
		...
		while (integration_time < D_Time)
		{
			...
			while (relaxation_time < Dt)
			{
				...
				while (dt_s_sum < dt)
				{
					...
					myocardium_activation.exec(dt_s);
			...
			fish_observer_contact_relation_fluid.updateConfiguration();
		}
		...
		pressure_probe_fish.writeToFile(number_of_iterations);
