 ...
 */
#include "2d_flow_stream_around_fish_reverse.h"
...
int main(int ac, char *av[])
{
    ...
    fish_observer.defineAdaptationRatios(1.15, 2.0);
    fish_observer.generateParticles<ObserverParticleGenerator>(observation_location);
    //----------------------------------------------------------------------
    ...
    //----------------------------------------------------------------------
    SimpleDynamics<TimeStepInitialization> initialize_a_fluid_step(water_block, makeShared<TimeDependentAcceleration>(Vec2d::Zero(), fish_body));
    ...
    InteractionDynamics<solid_dynamics::PressureForceAccelerationFromFluidRiemann> pressure_force_on_solid(fish_contact);
   ...
   //	New add
  //----------------------------------------------------------------------
    SimpleDynamics<solid_dynamics::FluidVelocityAroundSolid> fluid_velocity_around_solid(fish_contact, frequency_);
    SimpleDynamics<solid_dynamics::SolidTorque> solid_angular(fish_body, frequency_, 0.01);
    //----------------------------------------------------------------------
    ...
    fish_body.addBodyStateForRecording<Vecd>("SolidAverageVelocity");
    fish_body.addBodyStateForRecording<Real>("Theta");
    //----------------------------------------------------------------------
    ...
    ReducedQuantityRecording<ReduceDynamics<solid_dynamics::ElasticEnergy>>write_elastic_energy(io_environment, fish_body);
    ReducedQuantityRecording<ReduceDynamics<solid_dynamics::SolidKinecticEnergy>>write_fish_kinetic_energy(io_environment, fish_body);
    ...
    RegressionTestTimeAverage<ReducedQuantityRecording<ReduceDynamics<solid_dynamics::TotalForceFromFluid>>>
        write_total_pressure_force_on_insert_body(io_environment, pressure_force_on_solid, "TotalPressureForceOnSolid");
    RegressionTestTimeAverage<ReducedQuantityRecording<ReduceDynamics<solid_dynamics::TotalForceFromFluid>>>
    ...
    RegressionTestEnsembleAverage<ObservedQuantityRecording<Vecd>>
        write_fish_tail_displacement("Position", io_environment, fish_observer_contact);
    RegressionTestEnsembleAverage<ObservedQuantityRecording<Real>>
        write_fish_direction("Theta", io_environment, fish_observer_contact);
    RegressionTestEnsembleAverage<ObservedQuantityRecording<Real>>
        write_fish_direction_filter("Theta_filter", io_environment, fish_observer_contact);
    RegressionTestEnsembleAverage<ObservedQuantityRecording<Vecd>>
        write_fish_average_velocity("SolidAverageVelocity", io_environment, fish_observer_contact);
    //----------------------------------------------------------------------
    ...
    write_elastic_energy.writeToFile(0);
    write_fish_kinetic_energy.writeToFile(0);
    write_total_viscous_force_on_insert_body.writeToFile(0);
    write_total_pressure_force_on_insert_body.writeToFile(0);
    write_total_force_on_insert_body.writeToFile(0);
    write_fish_tail_displacement.writeToFile(0);
    write_fish_direction.writeToFile(0);
    write_fish_direction_filter.writeToFile(0);
    write_fish_average_velocity.writeToFile(0);
    //----------------------------------------------------------------------
    ...
    while (GlobalStaticVariables::physical_time_ < End_Time)
    {
        ...
        while (integration_time < D_Time)
        {
            ...
            while (relaxation_time < Dt)
            {
                ...
                solid_angular.exec(dt);
                fluid_velocity_around_solid.exec(dt);
        ...
        write_real_body_states.writeToFile();
        write_elastic_energy.writeToFile(number_of_iterations);
        write_fish_kinetic_energy.writeToFile(number_of_iterations);
        write_total_viscous_force_on_insert_body.writeToFile(number_of_iterations);
        write_total_pressure_force_on_insert_body.writeToFile(number_of_iterations);
        write_total_force_on_insert_body.writeToFile(number_of_iterations);
        write_fish_tail_displacement.writeToFile(number_of_iterations);
        write_fish_direction.writeToFile(number_of_iterations);
        write_fish_direction_filter.writeToFile(number_of_iterations);
        write_fish_average_velocity.writeToFile(number_of_iterations);
