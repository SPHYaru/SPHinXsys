 ...
 */
#include "2d_flow_stream_around_fish_rotation.h"
...
int main(int ac, char *av[])
{
    ...
    /* Yaru new add*/
    BodyAlignedBoxByParticle emitter_buffer(
        water_block, makeShared<AlignedBoxShape>(Transform(Vec2d(emitter_buffer_translation)), emitter_buffer_halfsize));
    SimpleDynamics<FreeStreamVelocity> emitter_buffer_inflow_condition(emitter_buffer, fish_body);
    SimpleDynamics<fluid_dynamics::EmitterInflowInjection> emitter_inflow_injection(emitter_buffer, 10, 0);

    SimpleDynamics<ModifiedDisposerOutflowDeletion2> disposer_outflow_deletion(water_block, fish_body, 0);
   ...
   // SimpleDynamics<fluid_dynamics::FreeStreamVelocityCorrection<FreeStreamVelocity>> velocity_boundary_condition_constraint(water_block);
    SimpleDynamics<FreeStreamCorrection> velocity_boundary_condition_constraint(water_block, fish_body);
   ...
   //----------------------------------------------------------------------
    /** Mass center .*/
    SimpleDynamics<solid_dynamics::ConstrainSolidBodyMassCenter> fish_body_mass_center(fish_body);
    ...
    fish_body.addBodyStateForRecording<Real>("Theta_filter");
    //----------------------------------------------------------------------
